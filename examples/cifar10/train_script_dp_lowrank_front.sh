...
# start training
if [ "$#" -ge "9" ]; then
	python python/lowrank_netsolver.py --solver ${solverfile} --weights ${orig_caffemodel} --device ${device_id} --ratio ${rank_ratio} --lra_type ${lra_type} --pruning_iter $9 
else
	python python/lowrank_netsolver.py --solver ${solverfile} --weights ${orig_caffemodel} --device ${device_id} --ratio ${rank_ratio}  --lra_type ${lra_type}
fi
